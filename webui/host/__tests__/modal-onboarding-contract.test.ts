...
describe('modal onboarding and credential contracts', () => {
  it('handles core-owned quick-start-request events in the renderer', () => {
    ...
    const quickStart = readSource('src', 'utils', 'quickStart.ts')
    ...
    assert.match(useSse, /showQuickStartOrCredentialFallback/)
    assert.match(quickStart, /ui\.openQuickStart\(request\)/)
  })
  ...
  it('keeps quick start out of minimal mode and falls back to credential completion', () => {
    ...
    assert.match(quickStart, /requestCredentials\(/)
    assert.match(quickStart, /missingUsername/)
    assert.match(quickStart, /missingPassword/)
    assert.match(quickStart, /ui\.openQuickStart\(request\)\s*return true/)
    assert.doesNotMatch(quickStartDialog, /saveSettings\(\{\s*minimal_mode:\s*false\s*\}\)/)
  })

  it('opens quick start after launch when fetched local user data is empty', () => {
    const app = readSource('src', 'App.vue')
    const useSse = readSource('src', 'composables', 'useSSE.ts')
    const quickStart = readSource('src', 'utils', 'quickStart.ts')

    assert.match(app, /requestQuickStartForMissingLocalData/)
    assert.match(app, /await Promise\.allSettled\(/)
    assert.match(app, /if \(authConfigReady && settingsConfigReady && requestQuickStartForMissingLocalData\(\)\) return/)
    assert.match(quickStart, /export function hasLocalUserData/)
    assert.match(quickStart, /auth\.username\.trim\(\)/)
    assert.match(quickStart, /auth\.password_stored/)
    assert.match(quickStart, /reason:\s*'missing'/)
    assert.match(quickStart, /distributionConfig\.defaultVpnServer/)
    assert.match(quickStart, /config\.settings\.minimal_mode/)
    assert.match(quickStart, /ui\.openQuickStart\(request\)/)
    assert.match(useSse, /showQuickStartOrCredentialFallback/)
    assert.doesNotMatch(useSse, /function handleQuickStartRequest/)
  })

  it('keeps quick start launch gating distinct from failed config loads and duplicate events', () => {
    const app = readSource('src', 'App.vue')
    ...
    assert.match(configStore, /authConfigLoaded/)
    assert.match(configStore, /authConfigLoadError/)
    assert.match(configStore, /authConfigLoaded\.value = true/)
    assert.match(configStore, /authConfigLoadError\.value = null/)
    assert.match(configStore, /authConfigLoadError\.value =/)
    assert.match(configStore, /return true/)
    assert.match(configStore, /return false/)

    assert.match(app, /const authConfigReady = authResult\.status === 'fulfilled' && authResult\.value === true/)
    assert.match(app, /const settingsConfigReady = settingsResult\.status === 'fulfilled' && settingsResult\.value === true/)
    assert.match(app, /if \(authConfigReady && settingsConfigReady && requestQuickStartForMissingLocalData\(\)\) return/)

    assert.match(uiStore, /quickStartDismissedStorageKey = 'exv:quick-start-dismissed'/)
    assert.match(uiStore, /quickStartDismissed = ref\(readQuickStartDismissed\(\)\)/)
    assert.match(uiStore, /function writeQuickStartDismissed\(value: boolean\)/)
    assert.match(uiStore, /function dismissQuickStart\(\)/)
    assert.match(uiStore, /quickStartDismissed\.value = true[\s\S]*writeQuickStartDismissed\(true\)/)
    assert.match(uiStore, /function completeQuickStart\(\)/)
    assert.match(uiStore, /function resetQuickStartDismissal\(\)/)
    assert.match(uiStore, /quickStartDismissed\.value = false[\s\S]*writeQuickStartDismissed\(false\)/)

    assert.match(quickStart, /if \(!config\.authConfigLoaded\) return false/)
    assert.match(quickStart, /if \(config\.authConfigLoadError\) return false/)
    assert.match(quickStart, /request\.reason === 'missing' && ui\.quickStartDismissed && hasLocalUserData\(config\.authConfig\)/)
    assert.doesNotMatch(quickStart, /if \(ui\.quickStartDismissed\) return false/)
    assert.match(quickStart, /if \(ui\.showQuickStart \|\| ui\.showCredentialPrompt\) return true/)
    assert.match(quickStart, /request\.reason === 'missing' && hasLocalUserData\(config\.authConfig\)/)

    assert.match(quickStartDialog, /ui\.dismissQuickStart\(\)/)
    assert.match(quickStartDialog, /ui\.completeQuickStart\(\)/)
    assert.match(settingsSystem, /await config\.resetConfig\(true\)[\s\S]*ui\.resetQuickStartDismissal\(\)/)
  })

  it('does not let quick start background refresh overwrite user edits', () => {
    const quickStartDialog = readSource('src', 'components', 'QuickStartDialog.vue')

    assert.match(quickStartDialog, /const formDirty = ref\(false\)/)
    assert.match(quickStartDialog, /function markFormDirty\(\)/)
    assert.match(quickStartDialog, /formDirty\.value = true/)
    assert.match(quickStartDialog, /if \(!ui\.showQuickStart \|\| formDirty\.value\) return/)
    assert.match(quickStartDialog, /@input="markFormDirty"/)
    assert.match(quickStartDialog, /@change="markFormDirty"/)
  })
  ...
  it('quick start supports quick/custom modes, import, skip, and service install default', () => {
    ...
    const globalStack = readSource('src', 'windows', 'GlobalWindowStack.vue')
    const modalShell = readSource('src', 'components', 'ModalShell.vue')
    const tokenInput = readSource('src', 'components', 'TokenInput.vue')

    assert.match(dialog, /mode = ref<'quick' \| 'custom'>\('quick'\)/)
    assert.match(dialog, /panelSize = computed\(\(\) => mode\.value === 'custom' \? 'xl' : 'md'\)/)
    assert.match(modalShell, /size\?: 'sm' \| 'md' \| 'lg' \| 'xl'/)
    assert.match(modalShell, /bodyScroll\?:\s*boolean/)
    assert.match(modalShell, /modal-shell__body--locked/)
    assert.match(modalShell, /\$slots\['header-start'\]/)
    assert.match(modalShell, /\$slots\['header-end'\]/)
    assert.match(modalShell, /\.modal-shell__panel--xl\s*\{[\s\S]*width:\s*min\(100%,\s*920px\)/)
    ...
    assert.doesNotMatch(dialog, /首次使用前补全连接信息/)
    assert.match(dialog, /quickStartDescription = computed\(\(\) =>[\s\S]*ui\.quickStartRequest\?\.reason === 'invalid'[\s\S]*:\s*''/)
    assert.match(dialog, /quick-start-dialog__quick-stack/)
    assert.match(dialog, /quick-start-dialog__custom-grid/)
    assert.match(dialog, /:close-on-scrim="false"/)
    assert.match(dialog, /:body-scroll="false"/)
    assert.match(dialog, /<template #header-end>(?:(?!<\/template>)[\s\S])*quick-start-dialog__mode-tabs/)
    assert.match(dialog, />\s*简要\s*</)
    assert.doesNotMatch(dialog, />\s*快速\s*</)
    assert.doesNotMatch(dialog, /<template #header-end>(?:(?!<\/template>)[\s\S])*quick-start-dialog__theme-control/)
    assert.doesNotMatch(dialog, /<div class="quick-start-dialog__topbar">/)
    assert.match(dialog, /quick-start-dialog__section--network/)
    assert.match(dialog, /quick-start-dialog__section--runtime/)
    assert.doesNotMatch(dialog, /quick-start-dialog__section--window/)
    assert.match(dialog, /grid-template-columns:\s*minmax\(0,\s*1fr\)\s+minmax\(0,\s*0\.85fr\)\s+minmax\(0,\s*1\.15fr\)/)
    assert.doesNotMatch(dialog, /<div v-if="mode === 'custom'" class="space-y-3">/)
    assert.match(dialog, /serverOptions: string\[\] = distributionConfig\.vpnServers\.map/)
    assert.match(dialog, /normalizeServerChoice\(server: string\)/)
    assert.match(dialog, /handleServerChoiceChange/)
    assert.match(dialog, /VPN 服务器/)
    assert.match(dialog, /rememberPassword = ref\(false\)/)
    assert.match(dialog, /launchAtLogin = ref\(false\)/)
    assert.match(dialog, /installService = ref\(true\)/)
    assert.match(dialog, /installService\.value = ui\.quickStartRequest\?\.defaults\.install_service \?\? true/)
    assert.match(dialog, /showInstallServiceOption = computed\(\(\) => !vpn\.serviceInstalled\)/)
    assert.match(dialog, /autoReconnect = ref\(true\)/)
    assert.match(dialog, /autoConnectOnLaunch = ref\(false\)/)
    assert.match(dialog, /minimizeToTrayOnConnect = ref\(false\)/)
    assert.match(dialog, /closePreference = ref<ClosePreference>\('smart'\)/)
    ...
    assert.match(dialog, /auto_reconnect:\s*autoReconnect\.value/)
    assert.match(dialog, /auto_connect_on_launch:\s*autoConnectOnLaunch\.value/)
    assert.match(dialog, /minimize_to_tray_on_connect:\s*minimizeToTrayOnConnect\.value/)
    assert.match(dialog, /window\.exv\?\.window\.setClosePreference\?\.\(closePreference\.value\)/)
    assert.match(dialog, /<label v-if="mode === 'custom'" class="quick-start-dialog__setting-row">[\s\S]*开机自启/)
    assert.match(dialog, /常用开关/)
    assert.match(dialog, /quick-start-dialog__personalization/)
    assert.match(dialog, /个性化/)
    assert.match(dialog, /<div v-if="mode === 'quick'" class="quick-start-dialog__personalization">/)
    assert.match(dialog, /quick-start-dialog__section--identity[\s\S]*关闭窗口行为[\s\S]*quick-start-dialog__personalization/)
    assert.match(dialog, /\.quick-start-dialog__theme-control\s*\{[\s\S]*display:\s*flex;[\s\S]*align-items:\s*center;[\s\S]*justify-content:\s*space-between;/)
    assert.match(dialog, /\.quick-start-dialog__accent-dots\s*\{[\s\S]*margin-left:\s*auto;/)
    assert.match(dialog, /\.quick-start-dialog__section--network\s*\{[\s\S]*align-self:\s*stretch;[\s\S]*grid-column:\s*3;[\s\S]*grid-row:\s*1;/)
    assert.doesNotMatch(dialog, /\.quick-start-dialog__section--network\s*\{[\s\S]*grid-column:\s*1 \/ -1;/)
    assert.match(dialog, /\.quick-start-dialog__route-block\s*\{[\s\S]*grid-template-rows:\s*auto minmax\(0,\s*1fr\);[\s\S]*align-content:\s*stretch;/)
    assert.match(dialog, /\.quick-start-dialog__route-input\s*\{[\s\S]*height:\s*100%;[\s\S]*min-height:\s*0;/)
    assert.match(dialog, /断线重连/)
    assert.match(dialog, /启动时自动连接/)
    assert.match(dialog, /连接后缩小到托盘区/)
    assert.match(dialog, /关闭窗口行为/)
    assert.match(dialog, /安装辅助服务/)
    assert.match(dialog, /v-if="showInstallServiceOption"/)
    assert.match(dialog, /v-model="installService"/)
    assert.doesNotMatch(dialog, /remember_password:\s*true/)
    assert.match(dialog, /if \(installService\.value\) \{[\s\S]*await vpn\.fetchServiceStatus\(\)[\s\S]*if \(installService\.value && !vpn\.serviceInstalled\)/)
    assert.match(dialog, /requestInstallService\(\{ confirmWhenInactive: false \}\)/)
    assert.match(dialog, /TokenInput/)
    assert.match(dialog, /placeholder="添加路由"/)
    assert.doesNotMatch(dialog, /添加路由后回车/)
    assert.match(tokenInput, /import \{ Plus, X \} from 'lucide-vue-next'/)
    assert.match(tokenInput, /class="token-input__tokens"/)
    assert.match(tokenInput, /class="token-input__field-row"/)
    assert.match(tokenInput, /class="token-input__field"/)
    assert.match(tokenInput, /class="token-input__add"/)
    assert.match(tokenInput, /aria-label="添加路由"/)
    assert.match(tokenInput, /@click="commitTokens\(pendingToken\)"/)
    assert.match(tokenInput, /event\.key === 'Enter'[\s\S]*commitTokens\(pendingToken\.value\)/)
    assert.match(tokenInput, /\.token-input--tokens\s*\{[\s\S]*align-content:\s*start;/)
    assert.match(tokenInput, /\.token-input__field-row\s*\{[\s\S]*min-height:\s*32px;/)
    assert.match(tokenInput, /@click\.stop="removeToken\(index\)"/)
    assert.doesNotMatch(tokenInput, /Backspace[\s\S]{0,160}removeToken/)
    assert.doesNotMatch(dialog, /settingsForm\.mtu/)
    assert.doesNotMatch(dialog, /settingsForm\.dtls/)
    assert.doesNotMatch(dialog, />MTU</)
    assert.doesNotMatch(dialog, /> DTLS</)
  ...
  it('renders minimal-window modals as compact text and small actions without scrolling forms', () => {
    ...
    const quickStartFlow = readSource('src', 'utils', 'quickStart.ts')
    const useSse = readSource('src', 'composables', 'useSSE.ts')
    ...
    assert.match(useSse, /showQuickStartOrCredentialFallback/)
    assert.match(quickStartFlow, /config\.settings\.minimal_mode/)
    assert.match(quickStartFlow, /requestCredentials\(/)
  ...
  it('documents class B private routes as the 172.16 through 172.31 range', () => {
    const routesSection = readSource('src', 'pages', 'settings', 'SettingsRoutesSection.vue')

    assert.match(routesSection, /处理所有 B 类内网地址（172\.16-172\.31 开头）/)
    assert.match(routesSection, /172\.16\.0\.0\/12/)
  })

  it('marks TAP as unavailable until the native packet backend exists', () => {
    const systemSection = readSource('src', 'pages', 'settings', 'SettingsSystemSection.vue')

    assert.match(systemSection, /tapNativeBackendImplemented = false/)
    assert.match(systemSection, /TAP 后端未接入 native 引擎/)
    assert.match(systemSection, /<option value="tap" disabled>TAP（后端未接入）<\/option>/)
    assert.match(systemSection, /:disabled="busyDriver === 'tap' \|\| !tapNativeBackendImplemented"/)
    assert.match(systemSection, /安装 TAP/)
  })
  ...
  it('splits settings into section components and keeps the shell as orchestration only', () => {
    ...
    assert.match(settings, /settings-page__content-shell/)
    assert.match(settings, /width:\s*90%/)
    assert.match(settings, /max-width:\s*none/)
    assert.match(settings, /<SettingsSectionAxis[\s\S]*\/>/)
  ...
  it('supports frontend-only light dark and system themes including quick start controls', () => {
    ...
    assert.match(quickStart, /useThemeStore/)
    assert.match(quickStart, /theme\.setThemeMode\(mode\)/)
    assert.doesNotMatch(quickStart, /config\.saveSettings\(\{[\s\S]*theme/)
  })
  ...
  it('offers per-theme accent palettes and applies accent tokens to the dashboard visual stage', () => {
    ...
    assert.match(css, /\[data-theme="light"\][\s\S]*--color-warning:\s*#C75A32/i)
  ...
  it('supports launch at login and gated auto connect on launch settings', () => {
    ...
    assert.match(configStore, /silent_startup:\s*boolean/)
    assert.match(configStore, /connection_state_notifications:\s*boolean/)
    assert.match(configStore, /retry_limit:\s*number/)
    assert.match(configStore, /launch_at_login:\s*false/)
    assert.match(configStore, /auto_connect_on_launch:\s*false/)
    assert.match(configStore, /silent_startup:\s*false/)
    assert.match(configStore, /connection_state_notifications:\s*false/)
    assert.match(configStore, /retry_limit:\s*0/)
    assert.match(settings, /launch_at_login:\s*false/)
    assert.match(settings, /auto_connect_on_launch:\s*false/)
    assert.match(settings, /silent_startup:\s*false/)
    assert.match(settings, /connection_state_notifications:\s*false/)
    assert.match(settings, /retry_limit:\s*0/)
    ...
    assert.match(system, /静默启动/)
    assert.match(system, /silent_startup/)
    assert.match(system, /silentStartupModel/)
    assert.match(system, /连接状态系统通知/)
    assert.match(system, /connection_state_notifications/)
    assert.match(system, /connectionStateNotificationsModel/)
    ...
    assert.match(connection, /重连尝试次数/)
    assert.match(connection, /min="0"/)
    assert.match(connection, /:disabled="!autoReconnectModel"/)
    assert.match(connection, /0 表示无限重连/)
    assert.match(connection, /关闭断线重连/)
    assert.doesNotMatch(connection, /-1 表示无限重连/)
    ...
    assert.match(registry, /settings\.silent_startup/)
    assert.match(registry, /settings\.connection_state_notifications/)
    assert.match(registry, /backendField:\s*'launch_at_login'/)
    assert.match(registry, /backendField:\s*'auto_connect_on_launch'/)
    assert.match(registry, /backendField:\s*'silent_startup'/)
    assert.match(registry, /backendField:\s*'connection_state_notifications'/)
    ...
    assert.match(app, /connection_state_notifications/)
    assert.match(app, /connectionNotificationState/)
    assert.match(app, /shell\?\.notify\?\.\(\{ title, body \}\)/)
    assert.match(quickStart, /auto_connect_on_launch:\s*autoConnectOnLaunch\.value/)
  })

  it('offers compact quick-start theme and accent controls', () => {
    const dialog = readSource('src', 'components', 'QuickStartDialog.vue')

    assert.match(dialog, /useThemeStore/)
    assert.match(dialog, /themeAccentPalettes/)
    assert.match(dialog, /type ThemeMode/)
    assert.match(dialog, /themeOptions/)
    assert.match(dialog, /selectQuickStartThemeMode\(option\.value\)/)
    assert.match(dialog, /theme\.setThemeMode\(mode\)/)
    assert.match(dialog, /activeAccentTheme/)
    assert.match(dialog, /activeAccentOptions/)
    assert.match(dialog, /selectQuickStartAccent\(option\.key\)/)
    assert.match(dialog, /theme\.setAccent\(activeAccentTheme\.value, accentKey\)/)
    assert.match(dialog, /quick-start-dialog__theme-control/)
    assert.match(dialog, /quick-start-dialog__theme-modes/)
    assert.match(dialog, /quick-start-dialog__accent-dots/)
    assert.match(dialog, /v-for="option in themeOptions"/)
    assert.match(dialog, />浅色</)
    assert.match(dialog, />深色</)
    assert.match(dialog, />系统</)
    assert.match(dialog, /v-for="option in activeAccentOptions"/)
    assert.match(dialog, /:style="\{ backgroundColor: option\.color \}"/)
    assert.doesNotMatch(dialog, /\{\{ option\.label \}\}/)
    assert.doesNotMatch(dialog, /\{\{ option\.description \}\}/)
  })
