...
describe('modal onboarding and credential contracts', () => {
  ...
  it('routes service install and uninstall through shared confirmation entrypoints', () => {
    const uiStore = readSource('src', 'stores', 'ui.ts')
    const confirmDialog = readSource('src', 'components', 'ConfirmDialog.vue')
    const vpnStore = readSource('src', 'stores', 'vpn.ts')
    const servicePage = readSource('src', 'pages', 'ServicePage.vue')
    const settingsSystem = readSource('src', 'pages', 'settings', 'SettingsSystemSection.vue')

    assert.match(uiStore, /interface ConfirmOptions/)
    assert.match(uiStore, /confirmConfirmLabel/)
    assert.match(uiStore, /confirmCancelLabel/)
    assert.match(uiStore, /confirmVariant/)
    assert.match(uiStore, /onCancel/)
    assert.match(confirmDialog, /ui\.confirmConfirmLabel/)
    assert.match(confirmDialog, /ui\.confirmCancelLabel/)
    assert.match(confirmDialog, /ui\.confirmVariant/)
    assert.match(vpnStore, /requestInstallService/)
    assert.match(vpnStore, /requestUninstallService/)
    assert.match(vpnStore, /安装服务必须先断开连接/)
    assert.match(vpnStore, /卸载服务必须先断开连接/)
    assert.match(vpnStore, /disconnectForServiceOperation/)
    assert.match(vpnStore, /function serviceOperationNeedsVpnDisconnect/)
    assert.doesNotMatch(vpnStore, /activeVpnWorkflow\(\)[\s\S]{0,260}status\.value\?\.process_running/)
    assert.doesNotMatch(vpnStore, /disconnectForServiceOperation[\s\S]{0,420}status\.value\?\.process_running/)
    assert.match(servicePage, /vpn\.requestInstallService/)
    assert.match(servicePage, /vpn\.requestUninstallService/)
    assert.match(settingsSystem, /vpn\.requestInstallService/)
    assert.match(settingsSystem, /vpn\.requestUninstallService/)
  })
  ...
  it('quick start supports quick/custom modes, import, skip, and service install default', () => {
    ...
    assert.match(dialog, /requestInstallService\(\{ confirmWhenInactive: false \}\)/)
  ...
  it('offers per-theme accent palettes and applies accent tokens to the dashboard visual stage', () => {
    const themeStore = readSource('src', 'stores', 'theme.ts')
    const personalization = readSource('src', 'pages', 'settings', 'SettingsPersonalizationSection.vue')
    const configStore = readSource('src', 'stores', 'config.ts')
    const css = readSource('src', 'style.css')
    const dashboard = readSource('src', 'pages', 'DashboardPage.vue')
    const visualStage = readSource('src', 'components', 'dashboard', 'DashboardVisualStage.vue')
    ...
    assert.match(dashboard, /DashboardVisualStage/)
    assert.match(visualStage, /var\(--color-warning\)/)
    assert.match(visualStage, /var\(--color-accent\)/)
    assert.match(visualStage, /var\(--color-warning-rgb\)/)
    assert.match(visualStage, /var\(--color-accent-rgb\)/)
    assert.doesNotMatch(dashboard, /padding-bottom:\s*5\.75rem/)
    assert.doesNotMatch(dashboard, /margin-top:\s*-12rem/)
    assert.doesNotMatch(dashboard, /DashboardTopologyMap/)
    assert.doesNotMatch(visualStage, /rgba\(245,\s*158,\s*11/)
    assert.doesNotMatch(visualStage, /rgb\(245[,\s]+158[,\s]+11/)
    assert.doesNotMatch(visualStage, /rgba\(34,\s*197,\s*94,\s*0\.78\)/)
    assert.doesNotMatch(visualStage, /rgb\(34\s+197\s+94\)/)
  })
