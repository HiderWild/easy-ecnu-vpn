...
describe('native WebView package policy', () => {
  ...
  it('keeps active packaging smoke and merge-prep scripts on WebView package paths', () => {
    ...
    const stableHelperResolverStart = windowsSmoke.indexOf('function Resolve-StableHelperPath')
    const stableHelperResolverEnd = windowsSmoke.indexOf(
      'function Convert-ServicePathNameToExecutablePath',
      stableHelperResolverStart,
    )
    const stableHelperResolver = windowsSmoke.slice(stableHelperResolverStart, stableHelperResolverEnd)
    ...
    assert.match(stableHelperResolver, /function Resolve-StableHelperPath/)
    assert.match(stableHelperResolver, /\[Environment\]::GetFolderPath\('CommonApplicationData'\)/)
    assert.match(stableHelperResolver, /if \(\[string\]::IsNullOrWhiteSpace\(\$programData\)\)/)
    assert.match(stableHelperResolver, /C:\\ProgramData\\EXV\\Helper\\exv-helper\.exe/)
    assert.doesNotMatch(stableHelperResolver, /\$env:ProgramData/)
    assert.doesNotMatch(stableHelperResolver, /LOCALAPPDATA|USERPROFILE/)
    assert.match(windowsSmoke, /\$svcQuery -and \$svcQuery\.Status -eq "Running"[\s\S]*Helper service binary path correct/)
    assert.match(windowsSmoke, /\$stableHelperServiceReady = \$false/)
    assert.match(windowsSmoke, /Existing host helper service uses a legacy path[\s\S]*SKIP/)
    assert.match(windowsSmoke, /Host helper service is not currently using a complete stable helper payload/)
  ...
  it('keeps Windows release packaging on native WebView package outputs', () => {
    ...
    const stableHelperCleanupStart = nsis.indexOf('Function un.RemoveStableHelperPayload')
    const stableHelperCleanupEnd = nsis.indexOf('FunctionEnd', stableHelperCleanupStart)
    const stableHelperCleanup = nsis.slice(stableHelperCleanupStart, stableHelperCleanupEnd)
    const elevatedHelperCleanupStart = nsis.indexOf('Function un.WriteHelperServiceMaintenanceScript')
    const elevatedHelperCleanupEnd = nsis.indexOf('FunctionEnd', elevatedHelperCleanupStart)
    const elevatedHelperCleanup = nsis.slice(elevatedHelperCleanupStart, elevatedHelperCleanupEnd)
    const legacyUserLocalHelperCleanupStart = nsis.indexOf('Function un.RemoveLegacyUserLocalHelperPayload')
    const legacyUserLocalHelperCleanupEnd = nsis.indexOf('FunctionEnd', legacyUserLocalHelperCleanupStart)
    const legacyUserLocalHelperCleanup = nsis.slice(
      legacyUserLocalHelperCleanupStart,
      legacyUserLocalHelperCleanupEnd,
    )

    assert.match(releaseScript, /\[string\]\$Version = ""/)
    assert.match(releaseScript, /\[switch\]\$DevBuild/)
    assert.doesNotMatch(releaseScript, /\[string\]\$BuildLabel = ""/)
    ...
    assert.match(releaseScript, /function Get-NextDevBuildNumber/)
    ...
    assert.match(releaseScript, /EXV-\$Version-dev\.\*-windows-x64-setup\.exe/)
    assert.match(releaseScript, /EXV-\$Version-dev\.\$next-windows-x64-setup\.exe/)
    ...
    assert.match(nsis, /Var StableHelperRoot/)
    assert.match(nsis, /Function un\.ResolveStableHelperRoot/)
    assert.match(nsis, /ReadEnvStr \$StableHelperRoot "ProgramData"/)
    assert.match(nsis, /StrCpy \$StableHelperRoot "C:\\ProgramData"/)
    assert.doesNotMatch(nsis, /\$COMMONAPPDATA/)
    assert.doesNotMatch(stableHelperCleanup, /\$%ProgramData%/)
    assert.match(nsis, /Call un\.ResolveStableHelperRoot/)
    assert.match(stableHelperCleanup, /Function un\.RemoveStableHelperPayload/)
    assert.match(stableHelperCleanup, /Stable helper payload is machine-scoped under ProgramData\./)
    assert.match(nsis, /Call un\.RemoveStableHelperPayload/)
    assert.match(stableHelperCleanup, /Delete "\$StableHelperRoot\\EXV\\Helper\\exv-helper\.exe"/)
    assert.match(stableHelperCleanup, /Delete "\$StableHelperRoot\\EXV\\Helper\\wintun\.dll"/)
    assert.match(stableHelperCleanup, /Delete "\$StableHelperRoot\\EXV\\Helper\\libgcc_s_seh-1\.dll"/)
    assert.match(stableHelperCleanup, /Delete "\$StableHelperRoot\\EXV\\Helper\\libstdc\+\+-6\.dll"/)
    assert.match(stableHelperCleanup, /Delete "\$StableHelperRoot\\EXV\\Helper\\libwinpthread-1\.dll"/)
    assert.match(stableHelperCleanup, /RMDir "\$StableHelperRoot\\EXV\\Helper"/)
    assert.match(stableHelperCleanup, /RMDir "\$StableHelperRoot\\EXV"/)
    assert.match(elevatedHelperCleanup, /Function un\.WriteHelperServiceMaintenanceScript/)
    assert.match(elevatedHelperCleanup, /\[Environment\]::GetFolderPath\('CommonApplicationData'\)/)
    assert.match(elevatedHelperCleanup, /\$\$stableHelperRoot = Join-Path \$\$programData 'EXV\\Helper'/)
    assert.match(elevatedHelperCleanup, /Join-Path \$\$stableHelperRoot 'exv-helper\.exe'/)
    assert.match(elevatedHelperCleanup, /Join-Path \$\$stableHelperRoot 'wintun\.dll'/)
    assert.match(elevatedHelperCleanup, /Join-Path \$\$stableHelperRoot 'libgcc_s_seh-1\.dll'/)
    assert.match(elevatedHelperCleanup, /Join-Path \$\$stableHelperRoot 'libstdc\+\+-6\.dll'/)
    assert.match(elevatedHelperCleanup, /Join-Path \$\$stableHelperRoot 'libwinpthread-1\.dll'/)
    assert.match(legacyUserLocalHelperCleanup, /Function un\.RemoveLegacyUserLocalHelperPayload/)
    assert.match(legacyUserLocalHelperCleanup, /Legacy user-local helper cleanup/)
    assert.match(nsis, /Call un\.RemoveLegacyUserLocalHelperPayload/)
    assert.match(legacyUserLocalHelperCleanup, /Delete "\$LOCALAPPDATA\\EXV\\Helper\\exv-helper\.exe"/)
    assert.match(legacyUserLocalHelperCleanup, /RMDir "\$LOCALAPPDATA\\EXV\\Helper"/)
    assert.match(legacyUserLocalHelperCleanup, /RMDir "\$LOCALAPPDATA\\EXV"/)
    ...
    assert.match(readme, /EXV-<version>-dev\.<n>-windows-x64-portable\.zip/)
    assert.match(readme, /EXV-<version>-dev\.<n>-windows-x64-setup\.exe/)
    assert.match(buildGuide, /CMakeLists\.txt/)
    assert.match(buildGuide, /-DevBuild/)
    assert.match(buildGuide, /EXV-3\.3\.7-dev\.1-windows-x64-setup\.exe/)
    assert.match(buildGuide, /RELEASE_NOTES_UNRELEASED\.md/)
  ...
  it('keeps start.ps1 desktop flow on native WebView shell paths', () => {
    ...
    const stableStartHelperStart = startCommon.indexOf('$programData =')
    const stableStartHelperEnd = startCommon.indexOf('$launchArgsFile =', stableStartHelperStart)
    const stableStartHelper = startCommon.slice(stableStartHelperStart, stableStartHelperEnd)
    ...
    assert.match(stableStartHelper, /\[Environment\]::GetFolderPath\('CommonApplicationData'\)/)
    assert.match(stableStartHelper, /if \(\[string\]::IsNullOrWhiteSpace\(\$programData\)\)/)
    assert.match(stableStartHelper, /'C:\\ProgramData\\EXV\\Helper\\exv-helper\.exe'/)
    assert.doesNotMatch(stableStartHelper, /\$env:ProgramData|LOCALAPPDATA|USERPROFILE/)
  ...
  it('keeps window mode resizing out of the settings persistence path', () => {
    ...
    assert.doesNotMatch(configStore, /window\.exv\.window\.setMode/)
	    assert.match(configStore, /'exv:minimal-mode'/)
	    assert.match(configStore, /'exv:minimize-to-tray-on-connect'/)
	    assert.match(configStore, /localStorage\.getItem\(/)
	    assert.match(configStore, /localStorage\.setItem\(/)
    assert.match(configStore, /delete remoteSettings\.minimal_mode/)
    assert.match(configStore, /delete remoteSettings\.minimize_to_tray_on_connect/)
  ...
  it('exposes one-shot native window actions for transparent shell transitions', () => {
    ...
    assert.match(types, /hideToTray\(\): Promise<\{ ok: true \}>/)
    ...
    assert.match(types, /notify\?\(payload: \{ title: string; body: string \}\): Promise<\{ ok: true \}>/)
    ...
    assert.match(win32Host, /hideToTray: \(\) => rpc\('window\.hideToTray'\)/)
    ...
    assert.match(win32Host, /notify: \(payload\) => rpc\('shell\.notify', payload \?\? \{\}\)/)
    assert.match(win32Host, /if \(action == "shell\.openExternal"\)[\s\S]*ShellExecuteW/)
    assert.match(win32Host, /if \(action == "shell\.notify"\)[\s\S]*show_tray_notification/)
    assert.match(win32Host, /Shell_NotifyIconW\(NIM_MODIFY, &notification\)/)
    ...
    assert.match(win32Host, /if \(action == "window\.hideToTray"\)[\s\S]*hide_to_tray\(\)/)
    ...
    assert.match(darwinHost, /hideToTray: \(\) => rpc\('window\.hideToTray'\)/)
    ...
    assert.match(darwinHost, /if \(action == "window\.hideToTray"\)[\s\S]*hide_to_status_item\(\)/)
  ...
  it('wraps advanced and minimal content in the shared transparent app frame', () => {
    ...
    assert.match(appVue, /function advancedPageShellClass/)
    assert.match(appVue, /routeName === 'dashboard'/)
    assert.match(appVue, /routeName === 'settings'/)
    assert.match(appVue, /mx-auto w-full max-w-5xl/)
    assert.match(appVue, /mx-auto w-\[90%\] max-w-none/)
    assert.match(appVue, /<div class="min-h-0 w-full flex-1 overflow-hidden px-4 py-4">/)
    assert.match(appVue, /<div :class="advancedPageShellClass\(viewRoute\.name\)">/)
  ...
  it('coordinates direction-specific transparent shell mode transitions in the renderer', () => {
    ...
    assert.match(frameVue, /mode-transition-surface/)
	    assert.match(frameRule, /--window-radius: 12px;/)
    ...
    assert.match(windowLayout, /kAppSurfaceAdvancedWindowBounds\{879, 563\}/)
    assert.match(windowLayout, /kAppSurfaceMinimalWindowBounds\{328, 136\}/)
  ...
  it('opens packaged no-argument launches visibly unless hidden startup is explicit', () => {
    ...
    assert.match(options, /options\.start_visibility_explicit = true/)
    assert.match(options, /if \(options\.start_visibility_explicit\) \{[\s\S]*packaged_options\.start_hidden = options\.start_hidden/)
    assert.match(win32Host, /desired_window_visible_ = !active_config_\.start_hidden/)
    assert.match(win32Host, /ShowWindow\(hwnd_, desired_window_visible_ \? SW_SHOWNORMAL : SW_HIDE\)/)
    assert.match(win32Host, /controller_->put_IsVisible\(desired_window_visible_ \? TRUE : FALSE\)/)
    assert.match(contract, /if \(sidecar_options\.start_hidden \|\| sidecar_options\.start_visibility_explicit\) \{\s*return 1;\s*\}/)
    assert.match(contract, /hide_window_options\.start_hidden/)
    assert.match(contract, /hide_window_options\.start_visibility_explicit/)
    assert.match(contract, /show_window_options\.start_visibility_explicit/)
  })
  ...
  it('enables WebView2 non-client drag regions for the Windows app frame', () => {
    ...
    assert.match(win32Host, /ICoreWebView2Settings3/)
    assert.match(win32Host, /put_AreBrowserAcceleratorKeysEnabled\(FALSE\)/)
    assert.match(win32Host, /configure_browser_accelerator_keys\(\)/)
  })
  ...
  it('treats accepted VPN connect jobs as cancellable frontend state', () => {
    ...
    assert.match(store, /function isFullStatusSnapshot\(data: Partial<VpnStatus>\)/)
    assert.match(store, /function reconcileAuthoritativeStatusEvent\(nextStatus: VpnStatus\)/)
    assert.match(store, /if \(connectInFlight\.value && isTerminalConnectStatus\(nextStatus\)\)/)
    assert.match(store, /if \(connectInFlight\.value && isTerminalConnectStatus\(nextStatus\)\) \{[\s\S]*finishConnectWorkflow\(\{ resetTrayHide: false \}\)/)
    assert.match(store, /if \(connectInFlight\.value && connectTerminal\) \{[\s\S]*finishConnectWorkflow\(\)/)
    assert.match(store, /if \(fullSnapshot\) \{[\s\S]*reconcileAuthoritativeStatusEvent\(nextStatus\)/)
    assert.match(store, /disconnectInFlight\.value &&[\s\S]*disconnectInFlight\.value = false/)
    assert.match(store, /stopConnectStatusPolling\(\)[\s\S]*stopAuthInteractionPolling\(\)/)
    assert.match(store, /function releaseDashboardConnectGuard\(\) \{[\s\S]*dashboardConnectInFlight\.value = false[\s\S]*\}/)
    assert.match(store, /function finishConnectWorkflow[\s\S]*releaseDashboardConnectGuard\(\)/)
    assert.match(store, /function handleStatusPollFailure[\s\S]*finishConnectWorkflow\(\)/)
    assert.match(store, /async function cancelConnect\(\)/)
    assert.match(store, /async function cancelConnect\(\)[\s\S]*finishConnectWorkflow\(\)/)
    ...
    assert.match(store, /async function connect\(providedPassword\?: string\)[\s\S]*if \(!acceptedByBackend\) \{[\s\S]*finishConnectWorkflow\(\)/)
    assert.match(store, /async function connectElevated[\s\S]*if \(!acceptedByBackend\) \{[\s\S]*finishConnectWorkflow\(\)/)
    assert.match(store, /async function connectFromDashboard[\s\S]*let acceptedConnectHandoff = false/)
    assert.match(store, /async function connectFromDashboard[\s\S]*finally \{[\s\S]*if \(!acceptedConnectHandoff\) \{[\s\S]*releaseDashboardConnectGuard\(\)/)
    assert.doesNotMatch(store, /async function connectFromDashboard[\s\S]*finally \{[\s\S]*dashboardConnectInFlight\.value = false[\s\S]*async function disconnectElevated/)
    assert.match(dashboard, /if \(vpn\.dashboardConnectGuardHeld\) return[\s\S]*if \(connecting\.value\) \{[\s\S]*vpn\.cancelConnect\(\)/)
  ...
  it('normalizes helper service maintenance results before chaining connect', () => {
    ...
    assert.match(store, /async function installServiceForConnection\(\)/)
    assert.match(store, /const installed = await requestInstallService\(\{ confirmWhenInactive: false \}\)[\s\S]*operation\?\.connect_can_continue[\s\S]*ui\.addToast\('辅助服务已安装但暂不可用，将使用一次性助手继续本次连接。', 'warning'\)[\s\S]*return true/)
    assert.match(store, /const canContinue = await installServiceForConnection\(\)[\s\S]*if \(!canContinue\) return[\s\S]*await fetchServiceStatus\(\)[\s\S]*await connect\(\)/)
    assert.match(store, /if \(serviceInstalled\.value && !serviceAvailable\.value\) \{[\s\S]*await repairService\(\)[\s\S]*await fetchServiceStatus\(\)/)
    assert.match(store, /if \(serviceInstalled\.value && !serviceAvailable\.value\) \{[\s\S]*canUseElevatedFallback\.value[\s\S]*await connectElevated\(\)[\s\S]*return/)
